	...
	<table class="table">
		...
		<c:forEach items="${stuCoursesPage.list}" var="course" varStatus="index">
			<tr>
				...
				<td><c:choose>
						<c:when test="${course.deletable}">
							<a href="DeleteSelectingCourse.htm?deletSelectingCourseID=${course.msc_id}&isChange=1">delete</a>
						</c:when>
	...
	<div class="pagination">
		<ul>
			...
			<c:if test="${stuCoursesPage.pageIndex > 1}">
				<li><a href="CourseSelectionFormOutcome?pageIndex=${stuCoursesPage.pageIndex - 1}">Prev</a></li>
			</c:if>
			<c:forEach var="i" begin="1" end="${stuCoursesPage.totalPage}">
				<li><a href="CourseSelectionFormOutcome.htm?pageIndex=${i}">${i}</a></li>
			</c:forEach>
			<c:if test="${stuCoursesPage.pageIndex < stuCoursesPage.totalPage}">
				<li><a href="CourseSelectionFormOutcome.htm?pageIndex=${stuCoursesPage.pageIndex + 1}">Next</a></li>
			</c:if>
		</ul>
	</div>
</div>
